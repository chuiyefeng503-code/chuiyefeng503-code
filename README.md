## Hi there 👋
# Hi there 👋 I'm [风吹叶]

研究兴趣：**隐私保护 (Privacy Protection)** 和 **函数秘密共享 (Function Secret Sharing)**。

### 🔗 找到我
*   **个人主页**: [chuiyefeng503-code.github.io](https://chuiyefeng503-code.github.io)
*   **Google Scholar**: [你的 Google Scholar 链接]
*   **Email**: [你的邮箱地址]

---

### 🔭 Currently working on
*   正在撰写关于**函数秘密共享**的安全计算、优化与应用的综述论文。
*   研究 **FSS (Function Secret Sharing)** 与神经网络结合的实现。

### 🌱 Learning & Research
*   关注领域：零知识证明 (ZKP)、私有信息检索 (PIR) 和同态秘密共享 (HSS)。
*   常用工具：Python, LaTeX/Overleaf, WSL2/Ubuntu。

### 💬 Ask me about
*   函数秘密共享 (Function Secret Sharing)
*   LLM 微调与 DPO 训练
<!--
**chuiyefeng503-code/chuiyefeng503-code** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
